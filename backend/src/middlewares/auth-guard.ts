...
export const authGuard = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!authHeader) {
    res.status(HttpStatus.UNAUTHORIZED).json({ error: 'Autenticação necessária.' });
    return; 
  }
  ...
  try {
    ...
    if (deletedAt) {
      res.status(HttpStatus.FORBIDDEN).json({ error: "Esta conta foi desativada e não pode ser utilizada." });
      return;
    }
    req.userId = user.id;
    next();
